...
public class DumpImgFactory {
	...
	public static Img< DoubleType > dumpPointClouds( final List< List< PointCloud2D< Double >>> clouds, final boolean interpolated ) {
		...
		final Img< DoubleType > ret = new ArrayImgFactory< DoubleType >().create( new long[] { sizeX, sizeY, 1, sizeZ + 1, clouds.size() }, new DoubleType() );
			...
			for ( final PointCloud2D< Double > slice : frame ) {
				for ( final Pixel2D< Double > point : slice.getPoints() ) {
					...
					if ( !interpolated ) {
						...
						System.err.println( String.format( "ArrayIndexOutOfBounds while dumping point-counds: %d, %d, 1, %d, %d", x, y, z, t ) );
					}
				}
				z++;
			}
	 ...
	 * @param clouds
	 */
	public static Img< DoubleType > dumpEllipticalShapes( final List< List< Ellipse2D >> shapes ) {
		double minX = Double.MAX_VALUE, maxX = Double.MIN_VALUE;
		double minY = Double.MAX_VALUE, maxY = Double.MIN_VALUE;
		int sizeZ = 0;
		...
		final Img< DoubleType > ret = new ArrayImgFactory< DoubleType >().create( new long[] { sizeX, sizeY, 1, sizeZ + 1, shapes.size() }, new DoubleType() );
